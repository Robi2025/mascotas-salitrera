...
const SESSION_DAYS = Math.max(1, Number(process.env.SESSION_DAYS || 3650));
...
app.post("/auth/resident",rate,async(req,res,next)=>{try{const base=userData({...req.body,email:"resident@resident.salitrera"});if(base.name.length<3||base.phone.length<7||!["Alemania","Britania"].includes(base.tower)||!base.department)return res.status(400).json({error:"invalid_user_data"});const secret=createSessionToken();const u={...base,email:`resident-${Date.now()}-${secret.slice(0,10)}@resident.salitrera`};const x=await run("INSERT INTO users (email,name,department,phone,tower,avatar_data,bio,role,password_hash) VALUES (?,?,?,?,?,?,?,'resident',?)",[u.email,u.name,u.department,u.phone,u.tower,u.avatar_data,u.bio,await hashPassword(`Aa1!${secret}`)]);const token=createSessionToken(),expires=new Date(Date.now()+SESSION_DAYS*86400000).toISOString();await run("INSERT INTO sessions (user_id,token_hash,expires_at) VALUES (?,?,?)",[x.id,hashToken(token),expires]);res.status(201).json({token,user:{id:x.id,...u,role:"resident",active:true}});}catch(e){next(e);}});
app.post("/auth/register",rate,async(req,res,next)=>{try{const u=userData(req.body),password=req.body?.password;if(!validUser(u))return res.status(400).json({error:"invalid_user_data"});if(!isStrongPassword(password))return res.status(400).json({error:"weak_password"});const x=await run("INSERT INTO users (email,name,department,phone,tower,avatar_data,bio,role,password_hash) VALUES (?,?,?,?,?,?,?,'resident',?)",[u.email,u.name,u.department,u.phone,u.tower,u.avatar_data,u.bio,await hashPassword(password)]);const token=createSessionToken(),expires=new Date(Date.now()+SESSION_DAYS*86400000).toISOString();await run("INSERT INTO sessions (user_id,token_hash,expires_at) VALUES (?,?,?)",[x.id,hashToken(token),expires]);res.status(201).json({token,user:{id:x.id,...u,role:"resident",active:true}});}catch(e){if(String(e.code).includes("CONSTRAINT")||e.code==="23505")return res.status(409).json({error:"email_exists"});next(e);}});
